...
class SyncCatalog(unittest.TestCase):
    def run_sync(self, invalid=False, payload=None, rename_service=False, shadow_siri=False, invalid_group=False):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if invalid_group:
                groups = root / 'config/proxy-groups.yaml'
                groups.write_text(groups.read_text().replace('name: "🍎 Apple"', 'name: "Missing Apple"'))
            ...
            if shadow_siri:
                bodies[privacy_url] += b'  - DOMAIN,guzzoni.apple.com\n'
            ...
            if invalid or payload is not None or rename_service or shadow_siri or invalid_group:
                self.assertEqual(result, 1)
                if shadow_siri:
                    self.assertIn('更早规则抢先匹配', errors.getvalue())
                if invalid_group:
                    self.assertIn('undefined policy groups', errors.getvalue())
    ...
    def test_siri_shadowing_preserves_all_existing_assets(self):
        self.run_sync(shadow_siri=True)

    def test_invalid_policy_group_preserves_all_existing_assets(self):
        self.run_sync(invalid_group=True)
